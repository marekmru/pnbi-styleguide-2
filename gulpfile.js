...
function watch() {
  gulp.watch(resolvePath(paths().source.patterns) + '/**/*.scss', { awaitWriteFinish: true }).on('change', gulp.series('sass'));
...
gulp.task('sass', function(){
  ...
  var injectFiles = gulp.src([
    path.join(resolvePath(paths().source.patterns), '/**/*.scss'),
    path.join('!' + resolvePath(paths().source.patterns) + '/00-base/fontawesome/*.scss'),
    path.join('!' + resolvePath(paths().source.patterns) + '/00-base/style.scss')
  ], { read: false });
